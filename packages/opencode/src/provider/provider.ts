  ...
  return {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from worldapp.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://worldapp.dev/",
            "X-Title": "worldapp",
            "X-Source": "worldapp",
          },
        },
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://worldapp.dev/",
            "X-Title": "worldapp",
            "X-BILLING-INVOKE-ORIGIN": "WorldApp",
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `worldapp/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `worldapp/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `worldapp auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `worldapp/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "worldapp",
          },
        },
      }),
    kilo: () =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          ...
          const parsed: Info = {
            ...
            source: "config",
            models: cfg.enabled_providers ? {} : (existing?.models ?? {}),
          }
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      const s = yield* InstanceState.get(state)
      const recent = cfg.enabled_providers
        ? []
        : yield* fs.readJson(path.join(Global.Path.state, "model.json")).pipe(
