...
const MANAGED_PLIST_DOMAIN = "dev.worldapp.managed"

// Keys injected by macOS/MDM into the managed plist that are not WorldApp config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/worldapp"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "worldapp")
    default:
      return "/etc/worldapp"
  }
}
