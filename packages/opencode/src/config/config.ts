...
const WORLDAPP_PROVIDER_ID = "world-app-technologies-api"
const WORLDAPP_AI_BASE_URL = process.env.WORLDAPP_AI_BASE_URL ?? "https://platform.worldapptechnologies.com/api/v1"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in WorldApp config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for WorldApp serve and web commands",
  }),
...
const WORLDAPP_DEFAULT_CONFIG: Info = {
  model: `${WORLDAPP_PROVIDER_ID}/nulu-4.8`,
  small_model: `${WORLDAPP_PROVIDER_ID}/nulu-4.8-flash`,
  enabled_providers: [WORLDAPP_PROVIDER_ID],
  share: "disabled",
  provider: {
    [WORLDAPP_PROVIDER_ID]: {
      name: "World App Technologies API",
      npm: "@ai-sdk/openai-compatible",
      api: WORLDAPP_AI_BASE_URL,
      env: ["WORLDAPP_API_KEY", "WORLDAPP_AI_API_KEY"],
      options: {
        baseURL: WORLDAPP_AI_BASE_URL,
      },
      models: {
        "nulu-4.8": {
          name: "Nulu 4.8",
...
function globalConfigFile() {
  const candidates = ["worldapp.jsonc", "worldapp.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      let result: Info = WORLDAPP_DEFAULT_CONFIG
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: CONFIG_SCHEMA }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "worldapp.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "worldapp.jsonc"), env))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/worldapp`
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("worldapp", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".worldapp") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["worldapp.json", "worldapp.jsonc"]) {
          ...
          // Auto-discovered plugins under `.worldapp/plugin(s)` are already local files, so ConfigPlugin.load
