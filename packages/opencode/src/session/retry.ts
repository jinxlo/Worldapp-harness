...
export const GO_UPSELL_URL = "https://worldapp.dev"
...
export function retryable(error: Err, provider: string) {
  ...
  if (SessionLegacy.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: "Contact WorldApp support to increase your model usage limit.",
          label: "subscribe",
          link: GO_UPSELL_URL,
        },
      }
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = workspace ? `https://worldapp.dev/workspace/${workspace}` : GO_UPSELL_URL
      return {
