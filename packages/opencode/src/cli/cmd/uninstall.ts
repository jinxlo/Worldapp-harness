...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall WorldApp and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall WorldApp")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g worldapp-ai",
      pnpm: "pnpm uninstall -g worldapp-ai",
      bun: "bun remove -g worldapp-ai",
      yarn: "yarn global remove worldapp-ai",
      brew: "brew uninstall worldapp",
      choco: "choco uninstall worldapp",
      scoop: "scoop uninstall worldapp",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "worldapp", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using WorldApp!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# WorldApp") || content.includes(".worldapp/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".worldapp/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".worldapp/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".worldapp"))
    ) {
      continue
    }
