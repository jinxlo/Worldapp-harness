  ...
  return [
    ...pipe(
      ...
      map((provider) => ({
        ...
        description: {
          "world-app-technologies-api": "(Recommended)",
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in worldapp.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          opencode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                World App Technologies API gives you access to Nulu 4.8 and Nulu 4.8 Flash with a single API key.
                key.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://platform.worldapptechnologies.com</span> to get a key
              </text>
            </box>
          ),
          "opencode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                World App Technologies API provides fast Nulu model access.
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://platform.worldapptechnologies.com</span> and create an API key
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in worldapp.json to use it.`,
          })
          dialog.clear()
          return
        }
