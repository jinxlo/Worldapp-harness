...
const TUI_SCHEMA_URL = "https://worldapp.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from worldapp.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "worldapp"),
    ...(await Filesystem.findUp(["worldapp.json", "worldapp.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "worldapp"))
  }
