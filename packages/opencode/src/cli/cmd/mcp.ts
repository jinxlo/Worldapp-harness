...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in worldapp.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .worldapp/ subdirectory too)
  const candidates = [path.join(baseDir, "worldapp.json"), path.join(baseDir, "worldapp.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".worldapp", "worldapp.json"), path.join(baseDir, ".worldapp", "worldapp.jsonc"))
  }
  ...
  // Default to worldapp.json if none exist
  return candidates[0]
}
